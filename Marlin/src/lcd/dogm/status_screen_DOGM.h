...
FORCE_INLINE void _draw_centered_temp(const int16_t temp, const uint8_t x,
                                      const uint8_t y)
{
    const char *const str = itostr3(temp);
    lcd_moveto(x - (str[0] != ' ' ? 0 : str[1] != ' ' ? 1 : 2) *
                       DOG_CHAR_WIDTH / 2,
               y);
    lcd_put_u8str(str);
                                      ...
                                      const bool blink)
{
    ...
    if (PAGE_CONTAINS(21, 28))
    {
        ...
        {
            const uint8_t h = isBed ? 7 : HEAT_INDICATOR_X, y = isBed ? 18 : 17;
            if (
#if HAS_HEATED_BED
        ...
        else
        {
            ...
            { // Draw a second dot if there's space
                ...
                if (--chars)
                {
                    ...
                    lcd_put_u8str_max(lcd_status_message,
                                      LCD_PIXEL_WIDTH -
                                          ((rlen + 2) * DOG_CHAR_WIDTH));
                }
            }
        }
        if (last_blink != blink)
...
// Heated bed
    ...
    //
    if (card.isFileOpen() &&
        PAGE_CONTAINS(42 - (TALL_FONT_CORRECTION), 51 - (TALL_FONT_CORRECTION)))
    {
        // Upper box
        ...
        // Bottom hollow box
        u8g.drawFrame(42, 49 - (TALL_FONT_CORRECTION), 10,
                      4); // 49-52 (or 48-51)
        // Corner pixel
...
#define PROGRESS_BAR_WIDTH (LCD_PIXEL_WIDTH - PROGRESS_BAR_X)
    ...
    if (PAGE_CONTAINS(49, 52 - (TALL_FONT_CORRECTION))) // 49-52 (or 49-51)
        u8g.drawFrame(PROGRESS_BAR_X, 49, PROGRESS_BAR_WIDTH,
                      4 - (TALL_FONT_CORRECTION));
    ...
    if (progress_bar_percent > 1)
    {
        ...
        if (PAGE_CONTAINS(50,
                          51 - (TALL_FONT_CORRECTION))) // 50-51 (or just 50)
            u8g.drawBox(PROGRESS_BAR_X + 1, 50,
                        (uint16_t)((PROGRESS_BAR_WIDTH - 2) *
                                   progress_bar_percent * 0.01),
                        2 - (TALL_FONT_CORRECTION));
...
#define SD_DURATION_X                                                          \
    (PROGRESS_BAR_X + (PROGRESS_BAR_WIDTH / 2) - len * (DOG_CHAR_WIDTH / 2))
#else
...
#define XYZ_FRAME_HEIGHT INFO_FONT_HEIGHT + 1
    ...
    if (page.page == 0)
    {
        ...
        strcpy(mstring,
               itostr3(100.0 * (parser.volumetric_enabled
    ...
    {
#if ENABLED(XYZ_HOLLOW_FRAME)
        u8g.drawFrame(0, XYZ_FRAME_TOP, LCD_PIXEL_WIDTH,
                      XYZ_FRAME_HEIGHT); // 8: 29-40  7: 29-39
#else
        u8g.drawBox(0, XYZ_FRAME_TOP, LCD_PIXEL_WIDTH,
                    XYZ_FRAME_HEIGHT); // 8: 30-39  7: 30-37
#endif
...
//
        ...
        lcd_moveto(47, 50);
        lcd_put_wchar(
            LCD_STR_FILAM_DIA[0]); // lcd_put_u8str_P(PSTR(LCD_STR_FILAM_DIA));
        lcd_moveto(93, 50);
...
#define STATUS_BASELINE (55 + INFO_FONT_HEIGHT)
    ...
    if (PAGE_CONTAINS(STATUS_BASELINE - (INFO_FONT_HEIGHT - 1),
                      STATUS_BASELINE))
    {

...
static millis_t progress_bar_ms =
    0; // Start millis of the current progress bar cycle
#if PROGRESS_MSG_EXPIRE > 0
static millis_t expire_status_ms =
    0; // millis at which to expire the status message
#endif
#endif
...
{
    byte temp[8];
    for (uint8_t i = 0; i < 8; i++) temp[i] = pgm_read_byte(&ptr[i]);
...
// CHARSET_BOOT
    ...
    // CHARSET_INFO
    const static PROGMEM byte bedTemp[8] = {B00000, B11111, B10101, B10001,
                                            B10101, B11111, B00000, B00000};

    const static PROGMEM byte degree[8] = {B01100, B10010, B10010, B01100,
                                           B00000, B00000, B00000, B00000};

    const static PROGMEM byte thermometer[8] = {B00100, B01010, B01010, B01010,
                                                B01010, B10001, B10001, B01110};

    const static PROGMEM byte uplevel[8] = {B00100, B01110, B11111, B00100,
                                            B11100, B00000, B00000, B00000};

    const static PROGMEM byte feedrate[8] = {B11100, B10000, B11000, B10111,
                                             B00101, B00110, B00101, B00000};

    const static PROGMEM byte clock[8] = {B00000, B01110, B10011, B10101,
    ...
    const static PROGMEM byte folder[8] = {B00000, B11100, B11111, B10001,
    ...
    { // Info Screen uses 5 special characters
        ...
        { // 3 Progress bar characters for info screen
            for (int16_t i = 3; i--;)
...
{
    lcd_moveto(0, line);
    for (uint8_t i = LCD_WIDTH + 1; --i;) lcd_put_wchar(' ');
}

// Scroll the PSTR 'text' in a 'len' wide field for 'time' milliseconds at position col,line
void lcd_scroll(const uint8_t col, const uint8_t line, const char *const text,
                const uint8_t len, const int16_t time)
{
...
static void logo_lines(const char *const extra)
{
    ...
    lcd_put_wchar('\x01');
    lcd_moveto(indent, 1);
    lcd_put_u8str_P(PSTR("|Marlin|"));
    lcd_put_u8str_P(extra);
    lcd_moveto(indent, 2);
    lcd_put_wchar('\x02');
    lcd_put_u8str_P(PSTR("------"));
    lcd_put_wchar('\x03');
                                   ...
                                   const bool blink)
{
    ...
    DISABLED(DISABLE_REDUCED_ACCURACY_WARNING)
...
FORCE_INLINE void _draw_heater_status(const int8_t heater, const char prefix,
                                      const bool blink)
{
#if HAS_HEATED_BED
    const bool  isBed = heater < 0;
    const float t1    = (isBed ? thermalManager.degBed()
                            : thermalManager.degHotend(heater)),
                t2    = (isBed ? thermalManager.degTargetBed()
                            : thermalManager.degTargetHotend(heater));
#else
    const float t1     = thermalManager.degHotend(heater),
                t2     = thermalManager.degTargetHotend(heater);
#endif

    if (prefix >= 0)
        lcd_put_wchar(prefix);
...
{
    const int16_t tix = (int16_t)(percent * (LCD_WIDTH)*3) / 100, cel = tix / 3,
                  rem = tix % 3;
    uint8_t i         = LCD_WIDTH;
...
//
    ...
    _draw_axis_value(X_AXIS,
                     ftostr4sign(LOGICAL_X_POSITION(current_position[X_AXIS])),
                     blink);

    lcd_put_wchar(' ');

    _draw_axis_value(Y_AXIS,
                     ftostr4sign(LOGICAL_Y_POSITION(current_position[Y_AXIS])),
                     blink);
    ...
    _draw_axis_value(Z_AXIS,
                     ftostr52sp(LOGICAL_Z_POSITION(current_position[Z_AXIS])),
                     blink);
...
// or if there is no message set.
...
#endif
    if (progress_bar_percent > 2 &&
        (ELAPSED(millis(), progress_bar_ms + PROGRESS_BAR_MSG_TIME) ||
    ...
    else
    {
        ...
        else
        {
            ...
            uint8_t chars =
                LCD_WIDTH - rlen; // Amount of space left in characters
            ...
            { // Draw a second dot if there's space
                ...
                if (--chars)
                    lcd_put_u8str_max(
                        lcd_status_message,
                        chars); // Print a second copy of the message
            }
        }
        if (last_blink != blink)
        {
            ...
            if (status_scroll_offset < slen)
            {
                ...
                while (!START_OF_UTF8_CHAR(
                    lcd_status_message[status_scroll_offset]))
                    status_scroll_offset++;
            }
            else
...
static void
lcd_implementation_hotend_status(const uint8_t row,
...
static void lcd_implementation_drawmenu_static(const uint8_t row,
                                               const char *  pstr,
...
static void lcd_implementation_drawmenu_generic(const bool    sel,
                                                const uint8_t row,
                                                const char *  pstr,
                                                const char    pre_char,
                                                const char    post_char)
{
...
static void lcd_implementation_drawmenu_setting_edit_generic(
    const bool sel, const uint8_t row, const char *pstr, const char pre_char,
...
#define DRAWMENU_SETTING_EDIT_GENERIC(_src)                                    \
    lcd_implementation_drawmenu_setting_edit_generic(sel, row, pstr, '>', _src)
#define DRAW_BOOL_SETTING(sel, row, pstr, data)                                \
    lcd_implementation_drawmenu_setting_edit_generic_P(                        \
        sel, row, pstr, '>', (*(data)) ? PSTR(MSG_ON) : PSTR(MSG_OFF))

void lcd_implementation_drawedit(const char *      pstr,
                                 const char *const value = NULL)
{
    ...
    if (value != NULL)
    {
        ...
        const uint8_t valrow =
            (utf8_strlen_P(pstr) + 1 + len + 1) > (LCD_WIDTH - 2)
                ? 2
...
static void lcd_implementation_drawmenu_sd(const bool sel, const uint8_t row,
                                           const char *const pstr,
                                           const char *      filename,
                                           char *const       longFilename,
    ...
    if (longFilename[0])
    {
        ...
        if (sel)
        {
            ...
            for (uint8_t l = FILENAME_LENGTH; l--;)
                name_hash = ((name_hash << 1) | (name_hash >> 7)) ^
                            filename[l]; // rotate, xor
            if (filename_scroll_hash != name_hash)
            {                                        // If the hash changed...
                filename_scroll_hash    = name_hash; // Save the new hash
                filename_scroll_max     = MAX(0, utf8_strlen(longFilename) -
                                                 n); // Update the scroll limit
...
static void lcd_implementation_drawmenu_sdfile(const bool    sel,
                                               const uint8_t row,
                                               const char *  pstr,
                                               const char *  filename,
                                               char *const   longFilename)
...
static void lcd_implementation_drawmenu_sddirectory(const bool    sel,
                                                    const uint8_t row,
                                                    const char *  pstr,
                                                    const char *  filename,
                                                    char *const   longFilename)
...
#define lcd_implementation_drawmenu_back(sel, row, pstr, dummy)                \
    lcd_implementation_drawmenu_generic(sel, row, pstr, LCD_UPLEVEL_CHAR,      \
                                        LCD_UPLEVEL_CHAR)
#define lcd_implementation_drawmenu_submenu(sel, row, pstr, data)              \
    lcd_implementation_drawmenu_generic(sel, row, pstr, '>',                   \
                                        LCD_STR_ARROW_RIGHT[0])
#define lcd_implementation_drawmenu_gcode(sel, row, pstr, gcode)               \
    lcd_implementation_drawmenu_generic(sel, row, pstr, '>', ' ')
#define lcd_implementation_drawmenu_function(sel, row, pstr, data)             \
    lcd_implementation_drawmenu_generic(sel, row, pstr, '>', ' ')
...
static uint8_t lcd_implementation_read_slow_buttons()
{
    ...
    if ((slow_bits & (B_MI | B_RI)) &&
        PENDING(millis(), next_button_update_ms)) // LCD clicked
        slow_bits &=
...
void add_edges_to_custom_char(custom_char *const custom, coordinate *const ul,
                              coordinate *const lr, coordinate *const brc,
                              const uint8_t cell_location);
...
coordinate pixel_location(int16_t x, int16_t y)
{
    ...
    xp =
        x -
        c * (ULTRA_X_PIXELS_PER_CHAR); // get the pixel offsets into the character cell
    xp = ULTRA_X_PIXELS_PER_CHAR - 1 -
         xp; // column within relevant character cell (0 on the right)
...
#define _ZLABEL(X, Y) _LABEL('Z', X, Y)
    ...
    /**
         ...
         */
    _XLABEL(_PLOT_X, 0);
    lcd_put_u8str(ftostr52(
        LOGICAL_X_POSITION(pgm_read_float(&ubl._mesh_index_to_xpos[x]))));

    _YLABEL(_LCD_W_POS, 0);
    lcd_put_u8str(ftostr52(LOGICAL_Y_POSITION(
        pgm_read_float(&ubl._mesh_index_to_ypos[inverted_y]))));
    ...
    uint8_t i, j, k, l, m, n, n_rows, n_cols, y, bottom_line, right_edge,
        x_map_pixels, y_map_pixels, pixels_per_x_mesh_pnt,
        pixels_per_y_mesh_pnt, suppress_x_offset = 0, suppress_y_offset = 0;
    ...
    x_map_pixels =
        ...
        (ULTRA_COLUMNS_FOR_MESH_MAP)-2; // minus 2 because we are drawing a box around the map
    y_map_pixels = (ULTRA_Y_PIXELS_PER_CHAR) * (ULTRA_ROWS_FOR_MESH_MAP)-2;

    pixels_per_x_mesh_pnt = x_map_pixels / (GRID_MAX_POINTS_X);
    pixels_per_y_mesh_pnt = y_map_pixels / (GRID_MAX_POINTS_Y);

    if (pixels_per_x_mesh_pnt >= ULTRA_X_PIXELS_PER_CHAR)
    { // There are only 2 custom characters available, so the X
    ...
    if (pixels_per_y_mesh_pnt >= ULTRA_Y_PIXELS_PER_CHAR)
    { // There are only 2 custom characters available, so the Y
    ...
    x_map_pixels =
        pixels_per_x_mesh_pnt *
        (GRID_MAX_POINTS_X); // now we have the right number of pixels to make both
    y_map_pixels =
        pixels_per_y_mesh_pnt * (GRID_MAX_POINTS_Y); // directions fit nicely

    right_edge = pixels_per_x_mesh_pnt * (GRID_MAX_POINTS_X) +
                 1; // find location of right edge within the character cell
    ...
    new_char.custom_char_bits[0] =
        0b11111U; // char #0 is used for the top line of the box
    lcd.createChar(0, (uint8_t *)&new_char);

    clear_custom_char(&new_char);
    k = (GRID_MAX_POINTS_Y)*pixels_per_y_mesh_pnt +
        1;                             // row of pixels for the bottom box line
    l = k % (ULTRA_Y_PIXELS_PER_CHAR); // row within relevant character cell
    new_char.custom_char_bits[l] =
        0b11111U; // char #1 is used for the bottom line of the box
    lcd.createChar(1, (uint8_t *)&new_char);

    clear_custom_char(&new_char);
    for (j = 0; j < ULTRA_Y_PIXELS_PER_CHAR; j++)
        new_char.custom_char_bits[j] =
            0b10000U; // char #2 is used for the left edge of the box
    lcd.createChar(2, (uint8_t *)&new_char);

    clear_custom_char(&new_char);
    m = (GRID_MAX_POINTS_X)*pixels_per_x_mesh_pnt +
        1; // Column of pixels for the right box line
    n = m % (ULTRA_X_PIXELS_PER_CHAR); // Column within relevant character cell
    i = ULTRA_X_PIXELS_PER_CHAR - 1 -
        n; // Column within relevant character cell (0 on the right)
    for (j = 0; j < ULTRA_Y_PIXELS_PER_CHAR; j++)
        new_char.custom_char_bits[j] =
            (uint8_t)_BV(i); // Char #3 is used for the right edge of the box
    ...
    add_edges_to_custom_char(&new_char, &upper_left, &lower_right,
                             &bottom_right_corner, TOP_LEFT);
    ...
    if (upper_left.column == lower_right.column - 1)
    {
        ...
        for (j = upper_left.y_pixel_offset; j < ypix; j++)
        {
            i = _BV(ULTRA_X_PIXELS_PER_CHAR -
                    1); // Fill in the left side of the right character cell
    ...
    {
        l = ULTRA_Y_PIXELS_PER_CHAR -
            upper_left
                .y_pixel_offset; // Number of pixel rows in top character cell
        k = pixels_per_y_mesh_pnt -
            l; // Number of pixel rows in bottom character cell
        clear_custom_char(&new_char);
        for (j = 0; j < k; j++)
        {
            i = upper_left.x_pixel_mask;
            for (m = 0; m < pixels_per_x_mesh_pnt; m++)
            { // Fill in the top side of the bottom character cell
                new_char.custom_char_bits[j] |= i;
                if (!(i >>= 1))
                    break;
            }
        ...
        add_edges_to_custom_char(&new_char, &upper_left, &lower_right,
                                 &bottom_right_corner, LOWER_LEFT);
    ...
    if (upper_left.column == lower_right.column - 1 &&
        upper_left.row == lower_right.row - 1)
    {
        l = ULTRA_Y_PIXELS_PER_CHAR -
            upper_left
                .y_pixel_offset; // Number of pixel rows in top character cell
        ...
        for (j = 0; j < k; j++)
        {
            ...
            i = _BV(ULTRA_X_PIXELS_PER_CHAR -
                    1); // Fill in the left side of the right character cell
            for (m = 0; m < pixels_per_x_mesh_pnt - 1 - l; m++)
            { // Fill in the top side of the bottom character cell
                new_char.custom_char_bits[j] |= i;
                i >>= 1;
            }
        ...
        add_edges_to_custom_char(&new_char, &upper_left, &lower_right,
                                 &bottom_right_corner, LOWER_RIGHT);
                              ...
                              uint8_t cell_location)
{
    ...
    /**
       ...
       */
    if (ul->row == 0 &&
        ((cell_location & TOP_LEFT) || (cell_location & TOP_RIGHT)))
    { // Only fill in the top line for the top character cells
        ...
        if (n_cols == 1)
        {
            if (ul->column != brc->column)
                ...
                for (i = brc->x_pixel_offset; i < ULTRA_X_PIXELS_PER_CHAR;
                     i++) // Single column on right side
                    SBI(custom->custom_char_bits[0], i);
        }
        else if (
            (cell_location & TOP_LEFT) ||
            lr->column !=
                brc->column) // Multiple column in the middle or with right cell in middle
    ...
    {
        if (ul->column == 0)
        { // Left column of characters on LCD Display
            k = ul->row == brc->row
                    ? brc->y_pixel_offset
    ...
    if (n_rows ==
            1 /* && (cell_location == TOP_LEFT || cell_location == TOP_RIGHT) */
        && ul->row == brc->row)
    {
    ...
    if (n_rows == 2 &&
        (cell_location == LOWER_LEFT || cell_location == LOWER_RIGHT) &&
        lr->row == brc->row)
    {
        if (n_cols == 1) // Double row, single column case
            k = ul->column == brc->column ? brc->x_pixel_mask : 0x01;
        else if (cell_location &
                 LOWER_RIGHT) // Double row, multiple column case
            k = lr->column == brc->column ? brc->x_pixel_mask : 0x01;
    ...
    if (lr->column == brc->column)
    {
        ...
        if (ul->column == brc->column || (cell_location & TOP_RIGHT) ||
            (cell_location & LOWER_RIGHT))
        {
